 ...
 */
 ...
 const createTweetElement = function(singleTweet) {
  ...
  const newTweeterParag = makeTweetParag(singleTweet.content.text);
  const fullFooter = makeFooter(singleTweet);
  return newArticle.append(fullHeader).append(newTweeterParag).append(fullFooter);
// })
 }
 ...
 const makeHeader =(singleTweet) => {
    ...
    return newTweeterHead.append(newTweeterHeadImg).append(newTweeterName).append(newTweeterUserName);
 }
 ...
 const makeFooter = (singleTweet) => {
  ...
  function renderTweets(tweets) {
    for(tweet in tweets) {
      ...
      console.log($('#tweet-container').append(currentArticle));
    }
